...
const matchMonth = (input) => {
  // Solution code here...
  const matchedInputs = ['October', 'Oct', 'october', 'oct']
  if (matchedInputs.indexOf(input) !== -1) {
    return true;
  } else {
    return false;
  }
...
const noPunctuation = str => {
  ...
  str.split(' ').forEach( word => {
    ...
    const lastLetterNumber = lastLetter.charCodeAt(0);
    if ((lastLetterNumber >= 65 && lastLetterNumber <=90) || (lastLetterNumber >= 97 && lastLetterNumber <= 122)) {
      result.push(word + ' ');
    }
   });
   return result;
